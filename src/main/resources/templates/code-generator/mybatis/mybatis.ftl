...
<mapper namespace="${packageName}.api.${classInfo.className}Mapper">

    <resultMap id="BaseResultMap" type="${packageName}.model.${classInfo.className}" >
        <#if classInfo.fieldList?exists && classInfo.fieldList?size gt 0>
            <#list classInfo.fieldList as fieldItem >
                <result column="${fieldItem.columnName}" property="${fieldItem.fieldName}" <#if fieldItem.fieldClass ="String"> jdbcType="VARCHAR" </#if> <#if fieldItem.fieldClass ="Date"> jdbcType="TIMESTAMP" </#if> <#if fieldItem.fieldClass ="Integer"> jdbcType="INTEGER" </#if>/>
            </#list>
        </#if>
    </resultMap>
